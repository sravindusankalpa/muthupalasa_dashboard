...
export function CrudInterface({ database, collection, title, description }: CrudInterfaceProps) {
  ...
  const fetchDocuments = async (page = 1, search = "", classification = "all") => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (classification !== "all" && (collection.includes("EventDaySubmission") || collection.includes("SC4"))) {
  ...
  const downloadImage = async (doc: Document, imageType?: 'imageUrl' | 'originalImageUrl' | 'backgroundMergedImage') => {
    ...
    } else if (collection.includes("EventDaySubmission") || collection.includes("SC4")) {
  ...
  const handleClassificationFilter = (value: string) => {
    ...
    if (value !== "all" && (collection.includes("EventDaySubmission") || collection.includes("SC4"))) {
  ...
  useEffect(() => {
    ...
    if (collection.includes("EventDaySubmission") || collection.includes("SC4")) {
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {(collection.includes("EventDaySubmission") || collection.includes("SC4")) && (
